...
const extractTceFunc = (body) => {
  try {
    const tceVariableMatcher = body.match(
      new RegExp(NEW_TCE_GLOBAL_VARS_REGEXP, "m"),
    );
...
const extractDecipherFunc = (body, name, code) => {
  try {
    ...
    const sigFunctionMatcher = body.match(
      new RegExp(TCE_SIGN_FUNCTION_REGEXP, "s"),
    );
    const sigFunctionActionsMatcher = body.match(
      new RegExp(TCE_SIGN_FUNCTION_ACTION_REGEXP, "s"),
    );
    ...
    if (sigFunctionMatcher && sigFunctionActionsMatcher && code) {
      resultFunc =
        ...
        DECIPHER_FUNC_NAME +
        "=" +
        sigFunctionMatcher[0] +
        sigFunctionActionsMatcher[0] +
        code +
        ";\n";
    ...
    const reverseKey = extractDollarEscapedFirstGroup(
      REVERSE_PATTERN,
      actionBody,
    );
    const sliceKey = extractDollarEscapedFirstGroup(SLICE_PATTERN, actionBody);
    const spliceKey = extractDollarEscapedFirstGroup(
      SPLICE_PATTERN,
      actionBody,
    );
...
const extractNTransformFunc = (body, name, code) => {
  try {
    ...
    if (nFunctionMatcher && name && code) {
      ...
      resultFunc =
        "var " + N_TRANSFORM_FUNC_NAME + "=" + nFunction + code + ";\n";
      return resultFunc + callerFunc;
    }
    ...
    const cleanedFunction = nFunction.replace(
      new RegExp(
        `if\\s*\\(typeof\\s*[^\\s()]+\\s*===?.*?\\)return ${paramName}\\s*;?`,
        "g",
      ),
      "",
    );
    ...
    resultFunc =
      tceVars + "var " + N_TRANSFORM_FUNC_NAME + "=" + cleanedFunction + ";\n";
    return resultFunc + callerFunc;
  } catch (e) {
  ...
  const nTransform = (url) => {
    try {
      ...
      if (transformedN) {
        if (n === transformedN) {
          console.warn(
            "Transformed n parameter is the same as input, n function possibly short-circuited",
          );
        } else if (
          transformedN.startsWith("enhanced_except_") ||
          transformedN.endsWith("_w8_" + n)
        ) {
        ...
        console.warn(
          "Transformed n parameter is null, n function possibly faulty",
        );
      }
